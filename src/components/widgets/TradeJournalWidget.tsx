/**
 * TradeJournalWidget — Full trade logging with automatic market context capture.
 * Each trade records regime, volatility, bias, sentiment, and session at entry.
 */
...
import type { MarketContextSnapshot } from "@/engines/tradePatternEngine";
...
const TradeJournalWidget = ({ config }: { config: any }) => {
  ...
  // Get current market context for snapshot
  ...
  // Fetch fear & greed and add to context
  ...
  useEffect(() => {
    supabase
      ...
      .single()
      .then(({ data }) => {
        if (data) setFearGreed(data.value);
      });
  }, []);

  const fullContext = useMemo(
    () => ({ ...currentContext, fear_greed: fearGreed }),
    [currentContext, fearGreed]
  );
  ...
  const handleAddTrade = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        market_context: fullContext as any,
        notes: notes || null,
        status: "open",
      })
      .select()
  ...
  }, [user, symbol, direction, entryPrice, notes, fullContext]);
    ...
    async (tradeId: string, exitPrice: string) => {
      ...
      if (!error) {
        setTrades((prev) =>
          prev.map((t) =>
            t.id === tradeId
              ? { ...t, exit_price: exit, pnl, pnl_percent: pnlPercent, outcome, status: "closed", exit_time: new Date().toISOString() }
              : t
          )
        );
        setExpandedId(null);
  ...
  return (
    <div className="h-full flex flex-col gap-2 overflow-hidden">
      ...
      {showForm && (
        <div className="p-2.5 rounded-lg bg-secondary/40 border border-border/30 space-y-2 shrink-0">
          ...
          <div className="flex flex-wrap gap-1">
            {fullContext.regime && (
              <Badge variant="outline" className="text-[8px] px-1.5 py-0 border-border/30">{fullContext.regime}</Badge>
            )}
            {fullContext.bias_label && (
              <Badge variant="outline" className="text-[8px] px-1.5 py-0 border-border/30">{fullContext.bias_label}</Badge>
            )}
            {fullContext.session && (
              <Badge variant="outline" className="text-[8px] px-1.5 py-0 border-border/30">{fullContext.session}</Badge>
            )}
            {fullContext.fear_greed != null && (
              <Badge variant="outline" className="text-[8px] px-1.5 py-0 border-border/30">F&G: {fullContext.fear_greed}</Badge>
            )}
          </div>
          <button
      ...
      <div className="flex-1 overflow-y-auto space-y-1.5 min-h-0 scrollbar-none">
        ...
        {openTrades.length > 0 && (
          <>
            <p className="text-[9px] font-bold uppercase tracking-widest text-warning/60 px-1">
        ...
        {closedTrades.length > 0 && (
          <>
            <p className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground/40 px-1 mt-2">
...
function TradeCard({
  ...
  const outcomeColor =
    trade.outcome === "win" ? "text-success" : trade.outcome === "loss" ? "text-destructive" : "text-muted-foreground";
  ...
  return (
    <div className="rounded-lg bg-secondary/20 border border-border/20 overflow-hidden">
      <button onClick={onToggle} className="w-full flex items-center gap-2 p-2 text-left">
        ...
        {isOpen && (
          <Badge variant="outline" className="text-[8px] px-1.5 py-0 border-warning/30 text-warning">OPEN</Badge>
        )}
      ...
      {expanded && (
        <div className="px-2 pb-2 space-y-2 border-t border-border/10">
          ...
          <div className="flex flex-wrap gap-1 mt-2">
            ...
            {ctx.confidence != null && <Badge variant="outline" className="text-[8px] px-1.5 py-0 border-border/30">Conf: {ctx.confidence}%</Badge>}
          </div>

          {trade.notes && (
            <p className="text-[9px] text-muted-foreground/60 italic">{trade.notes}</p>
          )}
